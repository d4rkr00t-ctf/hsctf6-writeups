We are given a [file](./data/reversed.txt). As the name suggests it, lets just reverse the file content and see.
```
$ python
>>> with open("reversed.txt", "rb") as f: data = f.read()
>>> print(data[::-1])
...
```
Our flag can be seen in reverse format in above text as `}𝚗𝚒𝚠_𝚎𝚑𝚝_𝚛𝚘𝚏_𝟾𝚏𝚝𝚞{𝚏𝚝𝚌𝚜𝚑`
